...
def _execute_inference_concurrently(
    ...
    user_simulator_config: Optional[types.UserSimulatorConfig] = None,
) -> list[
    Union[
...
async def _run_adk_user_simulation(
    row: pd.Series,
    agent: LlmAgent,
    config: Optional[types.UserSimulatorConfig] = None,
) -> list[dict[str, Any]]:
            ...
            events.append(
                {
                    ...
                    "timestamp": invocation.creation_timestamp,
                }
            )
...
def _run_agent_internal(
    ...
    for resp_item in raw_responses:
        ...
        if _is_multi_turn_agent_run(user_simulator_config, prompt_dataset):
            agent_data_row = {"turns": resp_item}
...
async def _execute_local_agent_run_with_retry_async(
    ...
    if user_simulator_config or "conversation_plan" in row:
        try:
            ...
            logger.error("User simulation failed: %s", e)
            return {
                "error": f"Multi-turn agent scraping with user simulation failed: {e}"
            }
